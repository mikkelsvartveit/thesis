...
class ColoredTestResult(unittest.TextTestResult):
    def addSuccess(self, test):
        self.stream.write(f'{Colors.OKGREEN}✓ {test.__class__.__name__}.{test._testMethodName}{Colors.ENDC}\n')
        
    def addError(self, test, err):
        self.stream.write(f'{Colors.FAIL}✗ ERROR: {test.__class__.__name__}.{test._testMethodName}{Colors.ENDC}\n')
        self.stream.write(f'{Colors.FAIL}{self._exc_info_to_string(err, test)}{Colors.ENDC}\n')
        
    def addFailure(self, test, err):
        self.stream.write(f'{Colors.FAIL}✗ FAIL: {test.__class__.__name__}.{test._testMethodName}{Colors.ENDC}\n')
        self.stream.write(f'{Colors.FAIL}{self._exc_info_to_string(err, test)}{Colors.ENDC}\n')
        
    def addSkip(self, test, reason):
        self.stream.write(f'{Colors.WARNING}⚠ SKIP: {test.__class__.__name__}.{test._testMethodName} ({reason}){Colors.ENDC}\n')
...
class TestPyTorchInstallation(unittest.TestCase):
    def test_pytorch_basic_imports(self):
        ...
        self.assertIsNotNone(torchvision.__version__, "TorchVision not properly installed")
        self.assertIsNotNone(torchaudio.__version__, "TorchAudio not properly installed")
    ...
    def test_accelerator_availability(self):
        ...
        if device_type == "cuda":
            print(f"{Colors.OKBLUE}CUDA Version: {Colors.BOLD}{torch.version.cuda}{Colors.ENDC}")
            print(f"{Colors.OKBLUE}GPU Device: {Colors.BOLD}{torch.cuda.get_device_name()}{Colors.ENDC}")
    ...
    def test_cuda_specific(self):
        ...
        self.assertGreaterEqual(torch.cuda.current_device(), 0, "No current CUDA device")
    ...
    def test_mps_specific(self):
        ...
        device = torch.device("mps")
        x = torch.randn(3, 3).to(device)
        self.assertEqual(x.device.type, "mps", "Tensor not properly moved to MPS device")
