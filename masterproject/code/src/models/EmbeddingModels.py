...
class EmbeddingAndCNNModel2(nn.Module):
    def __init__(
        self,
        num_classes=2,
        max_length=1024,
        ...
        super(EmbeddingAndCNNModel2, self).__init__()
        self.max_length = max_length

        # Embedding layer to convert byte integers into dense vectors
        ...
        # Global average pooling: Adaptive pooling to output a single value per channel.
    ...
    def forward(self, x):
        # x: (batch_size, max_length), where each value is a byte (0-255)
        # Embedding: resulting in (batch_size, max_length, embedding_dim)
        x = self.embedding(x)

        # PyTorch's Conv1d expects (batch, channels, sequence_length)
        # So we need to permute the embedding output from (B, L, C) to (B, C, L)
        x = x.permute(0, 2, 1)
        ...
        # Global Average Pooling: result in shape (B, 256, 1)
        x = self.global_pool(x)
        x = x.squeeze(-1)  # shape becomes (B, 256)

        # Fully connected layers
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        out = self.fc2(x)  # logits for each ISA class
