...
def train_test_separate_datasets(
    config,
    ...
    groups = list(map(lambda x: x["architecture"], training_dataset.metadata))
    target_features = list(
        map(lambda x: x[config["target_feature"]], training_dataset.metadata)
    )
    ...
    all_train_labels = [
        training_dataset.metadata[i][config["target_feature"]]
