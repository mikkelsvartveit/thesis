...
def kfold_cv(config, dataset: Dataset, model_class: nn.Module.__class__, device):

    has_wandb_project_name = (
        "wandb_project_name" in config["validator"]
        and config["validator"]["wandb_project_name"]
    )
    if not has_wandb_project_name:
    ...
    group_name = f"kfold {config['model']['name']} {config['target_feature']} {datetime.now().strftime('%H:%M:%S, %d-%m-%Y')}"

    wandb_project = config["validator"]["wandb_project_name"]

    target_features = [item[config["target_feature"]] for item in dataset.metadata]

    kfold = KFold(
        n_splits=config["validator"]["n_splits"], shuffle=True, random_state=seed
    )
    ...
    for train_idx, test_idx in kfold.split(range(len(dataset))):
        ...
        wandb_run = wandb.init(
            ...
            name=f"fold_{fold}",
            mode=(
                "online" if has_wandb_project_name else "disabled"
            ),  # disabled = no-op
        )
    ...
    wandb_run = wandb.init(
        ...
        mode=("online" if has_wandb_project_name else "disabled"),  # disabled = no-op
    )
    ...
    wandb_run.log(
        {
            ...
            "accuracies_per_fold": wandb.Table(
                data=[[fold, acc] for fold, acc in accuracies.items()],
    ...
    # Print final results
    print("\n=== Results ===")
    for fold, accuracy in accuracies.items():
        print(f"Fold {fold}: {100*accuracy:.2f}%")
