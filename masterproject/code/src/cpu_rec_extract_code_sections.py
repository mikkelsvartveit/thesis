...
class TrainingData(object):
    ...
    def add_training(self, arch, file=None, section="text", data=None, repeat=1):
        ...
        if data is None:
            ...
            if section is None:
                ...
                raise TypeError(
                    "Invalid type %s for section in add_training"
    ...
    def extract_section_elfesteem(data, section=False):
        ...
        if data.startswith(struct.pack("%dB" % len(magic), *magic)):
            ...
            for sh in e.sh:
                if (
                    section == "text" and sh.sh.name.startswith(".text")
    ...
    def build_default_corpus(self, basedir):
        ...
        self.add_training(
            "S-390", file=basedir + "ELF/s390x/libgmp.so.10.2.0.xz"
        )  # S/390
        ...
        self.add_training(
            "ARcompact", file=basedir + "CROSS_COMPILED/minigzip-arc-elf.xz"
        )
        self.add_training(
            "ARcompact", file=basedir + "CROSS_COMPILED/jpegtran-arc-elf.xz"
        )
        ...
        self.add_training(
            "Blackfin", file=basedir + "CROSS_COMPILED/minigzip-bfin-elf.xz"
        )
        self.add_training(
            "Blackfin", file=basedir + "CROSS_COMPILED/jpegtran-bfin-elf.xz"
        )
        self.add_training(
            "TMS320C2x",
            file=basedir + "TMS320/C28346_Load_Program_to_Flash.out",
            repeat=5,
        )
        self.add_training(
            "TMS320C6x", file=basedir + "CROSS_COMPILED/minigzip-c6x-elf.xz"
        )
        #       _training('TMS320C6x',     file = basedir+'CROSS_COMPILED/jpegtran-c6x-elf.xz') # does not build; bug of gcc for jccolor.o
        self.add_training(
            "CompactRISC", file=basedir + "CROSS_COMPILED/minigzip-cr16-elf.xz"
        )
        #       _training('CompactRISC',   file = basedir+'CROSS_COMPILED/jpegtran-cr16-elf.xz') # does not build; bug of gcc for jdmarker.o
        self.add_training(
            "AxisCris", file=basedir + "CROSS_COMPILED/minigzip-cris-elf.xz"
        )
        self.add_training(
            "AxisCris", file=basedir + "CROSS_COMPILED/jpegtran-cris-elf.xz"
        )
        self.add_training(
            "Epiphany", file=basedir + "CROSS_COMPILED/minigzip-epiphany-elf.xz"
        )
        #       _training('Epiphany',      file = basedir+'CROSS_COMPILED/jpegtran-epiphany-elf.xz') # does not build; bug of gcc for jidctred.o
        self.add_training(
            "FR30", file=basedir + "CROSS_COMPILED/minigzip-fr30-elf.xz"
        )  # Fujitsu FR30
        ...
        self.add_training(
            "FT32", file=basedir + "CROSS_COMPILED/minigzip-ft32-elf.xz"
        )  # FTDI FT32
        self.add_training("FT32", file=basedir + "CROSS_COMPILED/jpegtran-ft32-elf.xz")
        self.add_training(
            "H8-300", file=basedir + "CROSS_COMPILED/minigzip-h8300-elf.xz"
        )  # H8/300
        self.add_training(
            "H8-300", file=basedir + "CROSS_COMPILED/jpegtran-h8300-elf.xz"
        )  # H8/300
        ...
        self.add_training(
            "IQ2000", file=basedir + "CROSS_COMPILED/minigzip-iq2000-elf.xz"
        )  # Vitesse IQ2000
        self.add_training(
            "IQ2000", file=basedir + "CROSS_COMPILED/jpegtran-iq2000-elf.xz"
        )
        self.add_training(
            "Mico32", file=basedir + "CROSS_COMPILED/minigzip-lm32-elf.xz"
        )  # LatticeMico32
        self.add_training(
            "Mico32", file=basedir + "CROSS_COMPILED/jpegtran-lm32-elf.xz"
        )
        ...
        self.add_training(
            "MCore", file=basedir + "CROSS_COMPILED/minigzip-mcore-elf.xz"
        )  # aka. Motorola RCE
        self.add_training(
            "MCore", file=basedir + "CROSS_COMPILED/jpegtran-mcore-elf.xz"
        )
        self.add_training(
            "MicroBlaze", file=basedir + "CROSS_COMPILED/minigzip-microblaze-elf.xz"
        )
        self.add_training(
            "MicroBlaze", file=basedir + "CROSS_COMPILED/jpegtran-microblaze-elf.xz"
        )
        ...
        self.add_training(
            "MIPSeb", file=basedir + "CROSS_COMPILED/minigzip-mips1-elf.xz"
        )
        self.add_training(
            "MIPSeb", file=basedir + "CROSS_COMPILED/jpegtran-mips1-elf.xz"
        )
        self.add_training(
            "MIPSeb", file=basedir + "CROSS_COMPILED/minigzip-mips2-elf.xz"
        )
        self.add_training(
            "MIPSeb", file=basedir + "CROSS_COMPILED/jpegtran-mips2-elf.xz"
        )
        self.add_training(
            "MIPS16", file=basedir + "CROSS_COMPILED/minigzip-mips16-elf.xz"
        )
        self.add_training(
            "MIPS16", file=basedir + "CROSS_COMPILED/jpegtran-mips16-elf.xz"
        )
        self.add_training(
            "MMIX", file=basedir + "CROSS_COMPILED/minigzip-mmix-elf.xz", repeat=2
        )
        self.add_training(
            "MMIX", file=basedir + "CROSS_COMPILED/jpegtran-mmix-elf.xz", repeat=2
        )
        self.add_training(
            "MN10300", file=basedir + "CROSS_COMPILED/minigzip-mn10300-elf.xz"
        )  # Matsushita MN10300
        self.add_training(
            "MN10300", file=basedir + "CROSS_COMPILED/jpegtran-mn10300-elf.xz"
        )
        self.add_training(
            "Moxie", file=basedir + "CROSS_COMPILED/minigzip-moxie-elf.xz"
        )
        self.add_training(
            "Moxie", file=basedir + "CROSS_COMPILED/jpegtran-moxie-elf.xz"
        )
        self.add_training(
            "MSP430", file=basedir + "CROSS_COMPILED/minigzip-msp430-elf.xz"
        )
        ...
        self.add_training(
            "MSP430", file=basedir + "CROSS_COMPILED/libjpeg-msp430.o.text.xz"
        )
        self.add_training(
            "NDS32", file=basedir + "CROSS_COMPILED/minigzip-nds32le-elf.xz"
        )  # same statistics for nsd32le and nds32be
        self.add_training(
            "NDS32", file=basedir + "CROSS_COMPILED/jpegtran-nds32le-elf.xz"
        )
        self.add_training(
            "NIOS-II", file=basedir + "CROSS_COMPILED/minigzip-nios2-elf.xz"
        )
        self.add_training(
            "NIOS-II", file=basedir + "CROSS_COMPILED/jpegtran-nios2-elf.xz"
        )
        self.add_training(
        ...
        self.add_training(
            "RX", file=basedir + "CROSS_COMPILED/minigzip-rx-elf.xz", section="P"
        )
        self.add_training(
            "RX", file=basedir + "CROSS_COMPILED/jpegtran-rx-elf.xz", section="P"
        )
        ...
        self.add_training(
            "Cell-SPU", file=basedir + "CROSS_COMPILED/minigzip-spu-elf.xz"
        )
        self.add_training(
            "Cell-SPU", file=basedir + "CROSS_COMPILED/jpegtran-spu-elf.xz"
        )
        self.add_training(
        ...
        self.add_training(
            "Visium", file=basedir + "CROSS_COMPILED/minigzip-visium-elf.xz"
        )
        self.add_training(
            "Visium", file=basedir + "CROSS_COMPILED/jpegtran-visium-elf.xz"
        )
        self.add_training(
            "Stormy16", file=basedir + "CROSS_COMPILED/minigzip-xstormy16-elf.xz"
        )
        self.add_training(
            "Stormy16", file=basedir + "CROSS_COMPILED/jpegtran-xstormy16-elf.xz"
        )
        self.add_training(
            "Xtensa", file=basedir + "CROSS_COMPILED/minigzip-xtensa-elf.xz"
        )
        self.add_training(
            "Xtensa", file=basedir + "CROSS_COMPILED/jpegtran-xtensa-elf.xz"
        )
        ...
        self.add_training(
            "Cray",
            file=basedir + "c-kermit/cku189.unicos7y.xz",
            section=slice(0x0, 0x118000),
        )  # Educated guess for .text section
        self.add_training(
            "VAX",
            file=basedir + "c-kermit/cku192.bellv10-vax.xz",
            section=slice(0x0, 0x4F800),
        )  # Educated guess for .text section
        self.add_training(
            "PDP-11",
            file=basedir + "c-kermit/cku192.bsd211.xz",
            section=slice(0x0, 0x1F000),
        )  # Educated guess for .text section
        self.add_training(
            "ROMP",
            file=basedir + "c-kermit/cku192.rtaixc-2.2.1-rtpc.xz",
            section=slice(0x0, 0x6E000),
        )  # Educated guess for .text section
        self.add_training(
            "WE32000",
            file=basedir + "c-kermit/cku192.att3bx.xz",
            section=slice(0xD0, 0xD0 + 0x51944),
        )  # .text section defined in COFF
        self.add_training(
            "CLIPPER",
            file=basedir + "c-kermit/cku196.clix-3.1.xz",
            section=slice(0xE0, 0xE0 + 0x108DE0),
        )  # .text section defined in COFF
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x4800, 0x8800),
        )  # .text section seems to be scattered across the file
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0xB800, 0x18800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x1A800, 0x21800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x23800, 0x29000),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x2A800, 0x3A000),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x3E800, 0x47800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x4B800, 0x52000),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x54800, 0x59800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x60000, 0x6C000),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0x74800, 0x84000),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0xB4800, 0xC7800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0xCF000, 0xE4800),
        )
        self.add_training(
            "i860",
            file=basedir + "c-kermit/ckl196-i860-vos1333.hex.xz",
            section=slice(0xEB000, 0xFB000),
        )
        ...
        self.add_training(
            "8051", file=basedir + "CROSS_COMPILED/tu-arithmetic-mcs51.hex"
        )
        self.add_training(
            "8051", file=basedir + "CROSS_COMPILED/tu-tea-ds390.hex"
        )  # DS80C390 and DS80C400 are derived from 8051
        self.add_training(
            "8051", file=basedir + "CROSS_COMPILED/tu-arithmetic-ds390.hex"
        )
        self.add_training(
            "STM8", file=basedir + "CROSS_COMPILED/tu-tea-stm8.hex", repeat=5
        )
        self.add_training(
            "STM8", file=basedir + "CROSS_COMPILED/tu-arithmetic-stm8.hex", repeat=5
        )
        self.add_training(
            "68HC08",
            file=basedir + "CROSS_COMPILED/tu-tea-hc08-elf",
            section="CSEG",
            repeat=2,
        )
        self.add_training(
            "68HC08",
            file=basedir + "CROSS_COMPILED/tu-path-hc08-elf",
            section="CSEG",
            repeat=2,
        )
        self.add_training(
            "68HC08",
            file=basedir + "CROSS_COMPILED/tu-arithmetic-hc08-elf",
            section="CSEG",
            repeat=2,
        )
        self.add_training("68HC11", file=basedir + "CROSS_COMPILED/tu-path-m68hc11-elf")
        self.add_training(
            "68HC11", file=basedir + "CROSS_COMPILED/tu-arithmetic-m68hc11-elf"
        )
        self.add_training(
            "Z80", file=basedir + "CROSS_COMPILED/tu-tea-z80.hex", repeat=2
        )
        self.add_training(
            "Z80", file=basedir + "CROSS_COMPILED/tu-path-z80.hex", repeat=2
        )
        self.add_training(
            "Z80", file=basedir + "CROSS_COMPILED/tu-arithmetic-z80.hex", repeat=2
        )
        self.add_training(
            "TLCS-90", file=basedir + "CROSS_COMPILED/tu-tea-tlcs90.hex", repeat=2
        )
        self.add_training(
            "TLCS-90", file=basedir + "CROSS_COMPILED/tu-path-tlcs90.hex", repeat=2
        )
        self.add_training(
            "TLCS-90",
            file=basedir + "CROSS_COMPILED/tu-arithmetic-tlcs90.hex",
            repeat=2,
        )
        # PIC10 and PIC16 from http://www.pic24.ru/doku.php/en/osa/ref/examples/intro
        self.add_training(
            "PIC10", file=basedir + "PIC10/3leds_pic10f222.hex", repeat=10
        )
        ...
        self.add_training(
            "PIC24",
            file=basedir + "PIC24/bpv4_fw7.0_opt0_18092016.hex",
            section=slice(0x8830, 0x1D2E0),
        )
        ...
        self.add_training(
            "#6502#cc65", file=basedir + "CROSS_COMPILED/tu-tea-cc65", repeat=5
        )
        self.add_training(
            "#6502#cc65", file=basedir + "CROSS_COMPILED/tu-path-cc65", repeat=5
        )
        self.add_training(
            "#6502#cc65", file=basedir + "CROSS_COMPILED/tu-arithmetic-cc65", repeat=5
        )
        ...
        self.add_training(
            "6502",
            file=basedir + "6502/data-4000-efff.bin",
            section=slice(0x4000, 0x4542),
            repeat=5,
        )
        ...
        self.add_training(
            "H8S",
            file=basedir + "H8S/bridge7757.mot.bin",
            section=slice(0x210C, 0x1671E),
        )
        ...
        self.add_training(
            "78k",
            file=basedir + "Metz/MB50AF1_NikonV12.bin",
            section=slice(0x2BA, 0x2BA + 0x7D5A),
        )
        # TriCore is used in Volkswagen's ECU, cf. https://debugmo.de/2015/12/dieselgate/
        self.add_training(
            "TriCore",
            file=basedir + "Volkswagen/FL_03L906018HK_3533.bin",
            section=slice(0xA094C, 0x1EA48C),
        )
        # OCaml bytecode, having non-standard statistical properties.
        self.add_training(
            "OCaml", file=basedir + "OCaml/camlp4", section=slice(0x1A, 0xEC856)
        )
        log.info(
        ...
        log.debug(
            "CPUs known: %s",
            ", ".join(
                sorted(list(set([_ for _ in self.archs if not _.startswith("_")])))
            ),
        )
...
class MarkovCrossEntropy(object):
    ...
    def __init__(self, training, length=2, modulo=None, FreqVariant="A"):
        ...
        if FreqVariant == "A":
            ...
            raise ValueError("FreqVariant=%r" % FreqVariant)
    ...
    def merge(self, res, cpu, other):
        ...
        for idx, (cp, cn) in enumerate(res):
            ...
            if (
                len(r)
                and idx < len(res) - 1
                and probably_outlier(cn, pos, r[-1], res[idx + 1])
            ):
                r[-1][1] += cn
                continue
            r.append([cp, cn])
...
if __name__ == "__main__":
    ...
    while len(argv):
        if argv[0] == "-d":
            ...
            if not os.path.isdir(dumpdir):
                log.error(
                    "Directory %r should be created before running the tool", dumpdir
                )
                sys.exit(1)
        elif argv[0] == "-f":
            fast = True
    ...
    for f in argv:
        ...
        if not fast:
            ...
            sys.stdout.write(
                "%-20s%-10s" % ("chunk(%#x;%s)" % (2 * sz * cnt, cnt), cpu)
            )
        sys.stdout.write("\n")
        sys.stdout.flush()
    sys.exit(0)
        ...
        def scan_file(self, fp):
            ...
            for cpu, cnt in res:
                ...
                self.result(
                    offset=pos,
                    file=fp,
                    description="%s (size=%#x, entropy=%f)"
                    % (cpu, cnt, self.shannon(raw[pos : pos + cnt])),
                )
                pos += cnt
