...
In the \ac{LOGO CV} ISAdetect suite and the evaluation strategies using multiple datasets we see more varied and interesting results compared to K-fold. In terms of raw classification performance, individual model performance varies quite a lot depending on the experimental suite and datasets. On the \ac{LOGO CV} suite with ISAdetect, we are able to achieve **90.3%** average accuracy on endianness detection and **88.0%** accuracy on instruction width detection. Both of these scores were achieved with the Simple1d-E model. The performance on the other suites are not quite as convincing, as on the suites testing on CpuRec we see higher variability between runs with lower accuracies, and lower overall accuracy on the BuildCross suites.

### Endianness detection

With endianness detection we see the clear performance differences between embedding and non-embedding models. The embedding models perform better in 3 out of 4 test suites, with a clear gap in performance on \ac{LOGO CV} ISAdetect. The only exception is ISAdetect-BuildCross where both 1d and 2d non-embedding versions beat out their embedding counterparts, with a lot less variance in the reported average accuracy as well. Still, the large performance difference on \ac{LOGO CV} ISAdetect, and the gap in accuracy in ISAdetect-CpuRec and Combined-CpuRec experiments indicates that the embedding models are better suited for endianness detection. This is backed by the paired t-test results in \autoref{fig:paired-t-test-logo-endianness}, \autoref{fig:paired-t-test-isadetect-cpurec-endianness}, and \autoref{fig:paired-t-test-combined-cpurec-endianness}, where in these three suites, none of the non-embedding models are able to outperform any of the embedding models with a p-value of less than 0.05.

### Instruction width type detection

\label{overview-instruction-width-type}

Regarding instruction width type detection, it is less clear which variations of models perform best compared to endianness detection. In the ISAdetect-CpuRec suite the performance is not great, with results comparable to random guesswork hovering between **53.2%** and **60.1%** average accuracy across all. Just like with endianness detection, the two best performing suites are \ac{LOGO CV} ISAdetect and Combined-CpuRec. We also see here that non embedding models are seemingly best in the two worst performing suites, ISAdetect-CpuRec and ISAdetect-BuildCross as shown in the comparison in \autoref{fig:paired-t-test-isadetect-cpurec-instructionwidthtype}, and \autoref{fig:paired-t-test-isadetect-buildcross-instructionwidthtype}. Based on the testing suite quality on \ac{LOGO CV} ISAdetect and Combined-CpuRec we initially concluded that embedding models overall perform better, although with less performance differences across the top performing models, and not as pronounced as with some of the endianness detection suites.

However, one trend that we noticed is that the confusion matrices on two suites training on ISAdetect show that variable width architectures are often misclassified as fixed width architectures. In the ISAdetect-CpuRec confusion matrix in \autoref{fig:cm-isadetect-cpurec-instructionwidthtype} we see that **70.7%** of variable width architectures are classified as fixed, a significant error across all models, with a similar pattern with ISAdetect-BuildCross shown in \autoref{fig:cm-isadetect-buildcross-instructionwidthtype}. This suggests that instruction width type classification from training on ISAdetect specifically, does not transfer well to other unseen architectures.
...
- Limitations
  ...
  - Dependency on external toolchain (mikpe's GitHub)
  - --unresolved-symbols=ignore-all _skip
  - -O0 vs -O3 optimization _skip
 -->
