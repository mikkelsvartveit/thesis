...
Both of the feature-engineered methods have other advantages in training data requirements and complexity compared to our \ac{CNN} approach. The bigram feature was limited to 150 binaries per architecture for training due to memory constraints, but is still able to perform well with a lot smaller training dataset than what deep learning approaches would likely require. The 65,536 long feature vector does result in a significant count of learnable parameters depending on the classifier, and scikitlearns MLPClassifier with the default hidden layer size of 100 would have 6.56 million weights [@MLPClassifierScikitLearn]. This falls in between our simple CNN models of 150k-217k parameters and ResNet50 for reference. The EndiannessSignature feature on the other hand is very simple with an input feature vector of only 4 elements and is able to achieve similar- and often outperform the bigram feature. Andreassen's approaches are able to achieve similar performance on paper with a lot less training data and complexity [@Andreassen_Morrison_2024].

<!-- TODO: Compare bigram and embedding? -->

A possible explanation for the effectiveness of Andreassen's feature extraction methods is the use of entire binary files, as both the bigram-based features are able to gather statistical information from the entire binary. \acp{CNN} automatic feature extraction are limited to the input window of the model. While our \ac{CNN} approach might be better suited for smaller samples code sections due to this, Andreassen's approach is able to make accurate predictions using much simpler models and classification techniques [@Andreassen_Morrison_2024].

#### Instruction width type

There is no directly comparable experimental suite for instruction width type detection in Andreassen's work since he uses CpuRec for training and tests on CpuRec, ISAdetectFull, and ISAdetectCode. He does not train instruction width type detection on ISAdetect due to the lack of variety in training data, justified by his labeling only resulting in the 2 variable width architectures of amd64 and i386, both from the x86-family. His results are still impressive training on the more limited CpuRec dataset, with accuracies of 86.0% on the \ac{LOGO CV} CpuRec suite and 89.1% accuracy using a Random Forest classifier with his autocorrelation feature. While not directly comparable, our own \ac{LOGO CV} suite on ISAdetect perform on par in terms of accuracy on ISAdetect (88.0% with Simple1d-E). Meanwhile, our suite training on ISAdetect and testing on CpuRec perform significantly worse, with only the Combined suite resulting in comparable performance (82.9% with Simple2d-E).

Comparing model performance, training data, and model complexity, Andreassen presents three feature extraction methods: ByteDifferencePrimes, AutoCorrelation and Fourier. Andreassen does not explicitly detail the feature vector lengths of his models, but reading his thesis we are able to create likely estimates. ByteDifferencePrimes keeps track of distances between identical byte values in the binary, managing the first 50 prime factors of distances for each byte-value. This should result in a feature vector of $50 \cdot 256 = 12,800$ elements. The AutoCorrelation feature depends on the lag range parameter, computing autocorrelation for each value across that range. The optimal lag range depended on the classifier, but varied between 32 and 1024. The Fourier feature converts the binary into a frequency spectrum, and number of input features depends on the decided frequency range. Like with the AutoCorrelation feature, the optimal frequency range depended on the classifier, but varied between 16 to 512 resulting in $2 \cdot Frequency_Range + 1$ features. Both AutoCorrelation and Fourier results in models much smaller in complexity than our \acp{CNN}, with AutoCorrelation seemingly performing on par or better than our models. ByteDifferencePrimes is a lot more complex, and the larger classifiers are more comparable to our Simple \ac{CNN} models.
