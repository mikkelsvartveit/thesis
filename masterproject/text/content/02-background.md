...
Transfer learning is the concept of training a model for one purpose, and then re-using some or all of the trained weights for a different application. For instance, \ac{CNN} models trained on ImageNet – a large dataset of 3.2 million images – are readily available online [@ImageNet]. Transfer learning is useful in cases where there is not enough training data to train a model from scratch, or when there are limited amounts of time or computational resources available.
